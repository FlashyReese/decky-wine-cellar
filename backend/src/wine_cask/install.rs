...
impl WineCask {
    pub async fn install_compatibility_tool(&self, install: Install, peer_map: &PeerMap) {
        if let Some(mut queue_compatibility_tool) = look_for_compressed_archive(&install) {
            ...
            self.extract_generate_and_move(peer_map, &install, &mut queue_compatibility_tool, reader).await;
        } else {}
    }

    pub async fn extract_generate_and_move(&self, peer_map: &PeerMap, install: &Install, queue_compatibility_tool: &mut QueueCompatibilityTool, reader: Cursor<Vec<u8>>) {
        if let Some(temp_dir) = prepare_temp_directory() {
            let steam_compatibility_tools_directory = self.steam_util.get_steam_compatibility_tools_directory();
            ...
            if valid_directories.len() == 1 {
                ...
                let new_path = match queue_compatibility_tool.flavor {
                    ...
                    | CompatibilityToolFlavor::Boxtron => {
                        let new_folder_name = format!("{}{}", &queue_compatibility_tool.flavor, &install.release.tag_name);
                        generate_compatibility_tool_vdf(new_compat_tool_vdf, &new_folder_name, &format!("{} {}", &queue_compatibility_tool.flavor, &install.release.tag_name));
...
fn prepare_temp_directory() -> Option<PathBuf> {
    let temp_dir = PathBuf::from(
        env::var("DECKY_PLUGIN_RUNTIME_DIR")
            .unwrap_or("/tmp/decky-wine-cellar".to_string()),
    ).join("temp");
...
pub fn look_for_compressed_archive(install_request: &Install) -> Option<QueueCompatibilityTool> {
        ...
        .find(is_compressed)
    {
        return Some(QueueCompatibilityTool {
            ...
            state: QueueCompatibilityToolState::Waiting,
            compress_type: compress_type(&asset.clone()),
            progress: 0,
        });
    }
