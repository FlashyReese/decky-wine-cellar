 ...
 */
export async function GetAvailableCompatTools(appId: number): Promise<CompatToolInfo[]> {
    try {
        ...
        return response.map((tool: CompatToolInfo) => ({ ...tool })) as CompatToolInfo[];
    } catch (error) {
 ...
 */
export function SpecifyCompatToolWithInfo(appId: number, toolName: CompatToolInfo): void {
